        ...
        <div class="bg-white shadow-sm sm:rounded-lg p-6">
            <form action="{{ route('admin.questions.update', $question->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="question_text" rows="3" class="w-full border-gray-300 rounded-lg shadow-sm" required>{{ $question->question_text }}</textarea>
                </div>

                @foreach(['a', 'b', 'c', 'd'] as $opt)
                    <div class="mb-3">
                        <label class="block text-gray-700 font-semibold">Pilihan {{ strtoupper($opt) }}</label>
                        <input type="text" name="option_{{ $opt }}" class="w-full border-gray-300 rounded-lg shadow-sm"
                            value="{{ $question['option_'.$opt] }}" required>
                    </div>
                @endforeach
                ...
                <div class="mb-4">
                    ...
                    <select name="correct_answer" class="w-full border-gray-300 rounded-lg shadow-sm" required>
                        @foreach(['a','b','c','d'] as $ans)
                            <option value="{{ $ans }}" @if($question->correct_answer == $ans) selected @endif>
                                Pilihan {{ strtoupper($ans) }}
                            </option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="tag" class="w-full border-gray-300 rounded-lg shadow-sm" value="{{ $question->tag }}">
                </div>
